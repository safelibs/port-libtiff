...
pub(crate) fn free_ycbcr_tables(state: *mut TIFFYCbCrToRGB) {
    unsafe {
...
pub unsafe extern "C" fn safe_tiff_cielab_to_xyz(
    ...
    safe_tiff_cielab16_to_xyz(
        cielab,
        l.saturating_mul(257),
        a.saturating_mul(256),
        b.saturating_mul(256),
        x,
        y,
        z,
    );
...
pub(crate) fn safe_tiff_cielab16_to_xyz(
    cielab: *mut TIFFCIELabToRGB,
    l: u32,
    a: i32,
    b: i32,
...
pub unsafe extern "C" fn safe_tiff_cielab_to_rgb_init(
    ...
    unsafe {
        ...
        (*cielab).rstep =
            ((*cielab).display.d_YCR - (*cielab).display.d_Y0R) / (*cielab).range as f32;
        for i in 0..=CIELAB_TABLE_RANGE {
            (*cielab).Yr2r[i] = (*cielab).display.d_Vrwr as f32
                * ((i as f64 / (*cielab).range as f64).powf(gamma) as f32);
        }
...
pub unsafe extern "C" fn safe_tiff_ycbcr_to_rgb(
    ...
    unsafe {
        ...
        *r = clamp_int(i, 0, 255);
        let i = *(*ycbcr).Y_tab.add(y)
            + ((*(*ycbcr).Cb_g_tab.add(cb) + *(*ycbcr).Cr_g_tab.add(cr)) >> SHIFT);
        *g = clamp_int(i, 0, 255);
...
pub unsafe extern "C" fn safe_tiff_ycbcr_to_rgb_init(
    ...
    unsafe {
        ...
        let block_bytes = 1024 + 256 * size_of::<c_int>() * 2 + 256 * size_of::<i32>() * 3;
        ...
        let d2 = -fix(clamp_float(
            luma[0] * (2.0 - 2.0 * luma[0]) / luma[1],
            0.0,
            2.0,
        ));
        let d3 = fix(clamp_float(2.0 - 2.0 * luma[2], 0.0, 2.0));
        let d4 = -fix(clamp_float(
            luma[2] * (2.0 - 2.0 * luma[2]) / luma[1],
            0.0,
            2.0,
        ));
...
fn oog_encode(u: f64, v: f64) -> c_int {
    ...
    for (vi, row) in UV_ROWS.iter().enumerate() {
        ...
        while ui >= 0 {
            ...
            let ang =
                (100.0 * 0.499999999 / std::f64::consts::PI) * (v - V_NEU).atan2(u - U_NEU) + 50.0;
...
pub unsafe extern "C" fn safe_tiff_logluv32_from_xyz(xyz: *mut f32, method: c_int) -> u32 {
    unsafe {
        ...
        let mut ue = if u <= 0.0 {
            ...
            tiff_itrunc(UVSCALE * u, method) as u32
        };
        let mut ve = if v <= 0.0 {
            0
        } else {
            tiff_itrunc(UVSCALE * v, method) as u32
        };
        if ue > 255 {
            ue = 255;
        }
